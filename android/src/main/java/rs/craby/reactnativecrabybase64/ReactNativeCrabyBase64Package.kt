package rs.craby.reactnativecrabybase64
...
class ReactNativeCrabyBase64Package : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyReactNativeCrabyBase64_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-react-native-craby-base-64")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return ReactNativeCrabyBase64Package.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
